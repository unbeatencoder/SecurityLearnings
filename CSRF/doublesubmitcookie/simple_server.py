...
# Fixed demo token for simplified double-submit-cookie validation.
...
class SimpleHTTPServer(http.server.SimpleHTTPRequestHandler):
    ...
    def end_headers(self):
        ...
        if self.command == "GET" and self.path in ("/", "/index.html"):
            ...
            # Double-submit-cookie pattern: send CSRF token in cookie, too.
            self.send_header(
                "Set-Cookie",
                f"csrf_token={DEMO_CSRF_TOKEN}; Path=/; SameSite=None",
            )
        super().end_headers()
    ...
    def _get_cookie_value(self, cookie_name):
        """Extract a cookie value by name from Cookie header."""
    ...
    def do_POST(self):
        ...
        session_id = self._get_cookie_value("session")
        ...
        csrf_cookie_token = self._get_cookie_value("csrf_token")
        if (
            not submitted_csrf_token
            or not csrf_cookie_token
            or submitted_csrf_token != csrf_cookie_token
        ):
            self._send_json(
                403,
                {
                    "status": "error",
                    "message": "Forbidden: invalid or missing double-submit CSRF token",
                },
            )
            return
